...
/**
* @author 作者 :lqr
* @version 创建时间：2021年10月27日 下午2:40:24
* 类说明
*/
public  class OpenCVUtil {
    ...
    public static Mat binary (Mat mat){
    	Mat binary = new Mat();
        Imgproc.adaptiveThreshold(mat, binary, 255, Imgproc.ADAPTIVE_THRESH_MEAN_C, Imgproc.THRESH_BINARY, 15, 25);
        return binary;
    }
     ...
     * @return
     */
    public static Mat face(Mat mat, CascadeClassifier faceDetector){
...
//                System.getProperty("user.dir")+"\\opencv\\haarcascades\\haarcascade_frontalface_alt.xml");
        ...
        //参数设置为scaleFactor=1.1f, minNeighbors=4, flags=0 以此来增长识别人脸的正确率
     ...
     * 循环进行人脸识别
     * */
    public static Mat faceLoop(Mat src, CascadeClassifier faceDetector){
        Mat face=new Mat();
        //默认人脸识别失败时图像旋转90度
        int k=90;
        while (k>0){
            for(int i=0;i<360/k;i++){
                //人脸识别
                face= OpenCVUtil.face(src, faceDetector);
                if(face==null){
                    src = rotate3(src,k);
                }else{
                    break;
    ...
    /**
     * 8邻域降噪,又有点像9宫格降噪;即若是9宫格中心被异色包围，则同化
     * @param pNum 默认值为1
     */
    public static Mat navieRemoveNoise(Mat mat,int pNum) {
        ...
        // 若是一个点的周围都是白色的，而它确是黑色的，删除它
        for (j = 1; j < nHeight - 1; ++j) {
            for (i = 1; i < nWidth - 1; ++i) {
                nValue =  (int)mat.get(j, i)[0];
                if (nValue == 0) {
                    nCount = 0;
                    // 比较以(j ,i)为中心的9宫格，若是周围都是白色的，同化
                    for (m = j - 1; m <= j + 1; ++m) {
                        for (n = i - 1; n <= i + 1; ++n) {
        ...
        for (i = 0; i < nWidth; ++i) {
            for (j = 0; j < nHeight; ++j) {
                if ((int) mat.get(j, i)[0] == BLACK) {
                    //用不一样颜色填充链接区域中的每一个黑色点
                    //floodFill就是把一个点x的全部相邻的点都涂上x点的颜色，一直填充下去，直到这个区域内全部的点都被填充完为止
